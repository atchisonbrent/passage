...
def verify_html(expected, actual, csp, expected_csp):
    ...
    if (
        not allowed
        or any(not item.startswith("'sha256-") for item in allowed)
        or "script-src-elem" in directives
    ):
    ...
    for attrs, body in scripts:
        ...
        if len(attributes) != len(parser.attributes) or not set(attributes) <= {
            "src",
            "type",
            "integrity",
            "data-cf-beacon",
        ...
        if digest in allowed or any(
            item.strip("'") in (value or "") for item in allowed for value in attributes.values()
        ):
            raise ValueError("Additional script would be authorized")
        source = attributes.get("src")
        if source:
            if not re.fullmatch(
                r"https://static\.cloudflareinsights\.com/beacon\.min\.js(?:/v[0-9a-f]+)?", source
            ) or attributes.get("type") not in (None, "module"):
...
def verify(base="https://passage.batchison.dev/"):
    ...
    names = ["index.html", "stories/index.html", "data/manifest.json"] + [
        "data/" + p["file"] for p in manifest["activity"]
    ]
    ...
    for name in names:
        ...
        if name.endswith(".html"):
            req = urllib.request.Request(
                base + route, headers={"User-Agent": "Passage-Snapshot/1.0"}
            )
            ...
            hashlib.sha256(request_bytes(base + route)).digest()
            != hashlib.sha256(expected).digest()
        ):
            raise ValueError("Public bytes differ: " + name)
    for script in re.findall(
        r"<script>(.*?)</script>", (ROOT / "public/index.html").read_text(), re.S
    ):
...
def main():
    ...
    parser.add_argument(
        "--base",
        default="https://passage.batchison.dev/",
        choices=["https://passage.batchison.dev/", "http://127.0.0.1:8653/"],
    )
