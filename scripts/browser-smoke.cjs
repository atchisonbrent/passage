...
async function until(fn) {
  ...
  throw Error('Browser readiness timeout: ' + (last?.message || ''));
}
(async () => {
  const proc = spawn(
    chrome,
  ...
  try {
    ...
    const navigate = async (url) => {
      ...
      await until(() => js('!!window.passageSnapshot?.().ready'));
    };
    ...
    for (const [width, height] of [
      ...
      await navigate(base);
      await checkControls({ call, js, click, delay, out, width, height });
      await click('#eventsView');
      assert.equal(await js("document.getElementById('eventCatalog').open"), true);
      assert.ok(
        await js("document.getElementById('freshness').textContent.includes('Observations:')"),
      );
      assert.ok(
        await js(
          "document.getElementById('shiftPeriodNote').textContent.includes('relative to observed coverage')",
        ),
      );
      await js(
      ...
      fs.writeFileSync(path.join(out, `analysis-${width}.png`), Buffer.from(image.data, 'base64'));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ results, discoveryCount: ids.length, errors }, null, 2),
    );
    console.log(
      JSON.stringify({
        viewports: results.length,
        discoveries: ids.length,
