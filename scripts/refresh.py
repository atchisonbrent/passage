...
def merge_rows(old, fresh, identifiers, start, end):
    ...
    for row in fresh:
        ...
        if any(v is not None and (isinstance(v, bool) or not isinstance(v, (int, float)) or not math.isfinite(v) or v < 0) for v in row[2:]):
            raise ValueError('Invalid source measurement')
        key = tuple(row[:2])
        prior=previous.get(key)
        if prior and any(before is not None and after is None for before,after in zip(prior[2:],row[2:])):
...
SERVICES = {
    'Daily_Ports_Data': ['portcalls','portcalls_tanker','portcalls_container','portcalls_dry_bulk','import','export',None],
    'Daily_Chokepoints_Data': ['n_total','n_tanker','n_container','n_dry_bulk',None,None,'capacity'],
}
...
def request_bytes(url):
    for attempt in range(3):
        try:
            req=urllib.request.Request(url,headers={'User-Agent':'Passage-Snapshot/1.0','Cache-Control':'no-cache'})
...
def query(service, **params):
    return BASE+service+'/FeatureServer/0/query?'+urllib.parse.urlencode({'f':'json',**params})


def cutoffs():
    return {service: request(query(service,where='1=1',outStatistics=json.dumps([{'statisticType':'max','onStatisticField':'date','outStatisticFieldName':'latest'}])))['features'][0]['attributes']['latest'] for service in SERVICES}
...
def acquire(service,start,end):
    ...
    if not isinstance(stamp,int):raise ValueError('Missing source revision stamp')
    ...
    for offset in range(0,count,1000):
        result=request(query(service,where=where,outFields='ObjectId,date,portid,'+','.join(f for f in fields if f),returnGeometry='false',orderByFields='ObjectId ASC',resultOffset=offset,resultRecordCount=1000))
    ...
    if len(rows)!=count or request(query(service,where=where,returnCountOnly='true'))['count']!=count:
...
def refresh(data,fetch,latest,today,full=False):
    ...
    for service in SERVICES:
        ...
        ids={p['id'] for p in catalog if (p['kind']=='chokepoint')==(service=='Daily_Chokepoints_Data')}
        old=[r for r in existing if r[0] in ids]
        start=manifest['start'] if full else max(manifest['start'],(date(manifest[service+'_latest'])-dt.timedelta(days=89)).isoformat())
    ...
    receipt={'changed':changed,'checked':today,'windows':windows,'full_reconciliation':full}
    if not changed:
        return receipt
    ...
    for month,rows in sorted(partitions.items()):
        ...
        manifest['activity'].append({'file':name,'rows':len(rows),'bytes':len(raw),'sha256':hashlib.sha256(raw).hexdigest()})
    for service in SERVICES:
        manifest[service+'_latest']=latest[service]
        manifest['rows'][service]=sum(1 for r in merged if r[0].startswith('chokepoint')==(service=='Daily_Chokepoints_Data'))
...
def published_baseline(data):
    ...
    for part in current['activity']:
        ...
        if hashlib.sha256(raw).hexdigest()!=part['sha256'] or len(json.loads(raw))!=part['rows']:
