...
def collect(service, where, fields):
    ...
    def page(offset):
        return request(query(service,where=where,outFields=fields+',ObjectId',returnGeometry='false',orderByFields='ObjectId ASC',resultType='standard',resultRecordCount=page_size,resultOffset=offset))['features']
    rows=[]
...
def run():
    ...
    for ident, rows in sorted(data.items()):
        files[ident] = {**save('history-'+ident+'.json', [[d,*v] for d,v in sorted(rows.items())]),'rows':len(rows)}
    result = {'assembled':dt.datetime.now(dt.timezone.utc).isoformat(),'start':'2019-01-01','end':'2025-12-31','files':files,'rows':totals,'sources':{s:query(s,where="date >= DATE '2019-01-01' AND date < DATE '2026-01-01'",outFields='*') for s in mapping},'cache_note':'Assembly time; pages may be reused from the manual acquisition cache. Not a new retrieval time for each row.'}
