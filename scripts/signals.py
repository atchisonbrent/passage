...
def strongest(rows, ending_since=None):
    for row in rows:
        if (
            ...
            or dt.date.fromisoformat(row[0]).isoformat() != row[0]
        ):
    ...
    for i in range(34, len(rows)):
        ...
        if abs(percent) < 50 or not (
            all(v < reference for v in values[28:]) or all(v > reference for v in values[28:])
        ):
            continue
        candidate = dict(
            start=window[28][0],
            end=window[-1][0],
            referenceStart=window[0][0],
            referenceEnd=window[27][0],
            mean=mean,
            reference=reference,
            percent=percent,
...
def discover(rows, catalog, contexts, archive, cutoff):
    ...
    for ident, observations in rows.items():
        for hit, is_recent in [
            (strongest(observations), False),
            (recent(observations, cutoff), True),
        ]:
            if hit:
                key = ident + ":" + hit["start"]
    ...
    for key, saved in sorted(keys.items()):
        ...
        if (
            ident not in catalog
            or key != ident + ":" + start
            or (dt.date.fromisoformat(end) - dt.date.fromisoformat(start)).days != 6
        ):
        ...
        valid = (
            ...
            and len({d for d, v in observed}) == 35
        )
        ...
        context = next(
            (
                c
                for c in contexts
                if c["place"] == ident and c["start"] == start and c["end"] == end
            ),
            None,
        )
        output.append(
            dict(
                id=key,
                place=ident,
                name=catalog[ident]["name"],
                start=start,
                end=end,
                referenceStart=rs,
                referenceEnd=(dt.date.fromisoformat(start) - dt.timedelta(days=1)).isoformat(),
                mean=mean,
                reference=reference,
                difference=difference,
                percent=difference / reference * 100 if reference else None,
                context=context,
                qualifies=hit is not None,
                recent=key in recent_ids,
            )
        )
        ...
        if hit:
            context = next(
                (
                    c
                    for c in contexts
                    if c["place"] == id and c["start"] == hit["start"] and c["end"] == hit["end"]
                ),
                None,
            )
            output.append(
