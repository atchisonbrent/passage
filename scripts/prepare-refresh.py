...
def validate_changed_paths():
    changed=subprocess.check_output(['git','status','--porcelain=v1','-z','--untracked-files=all'],cwd=ROOT).decode().split('\0')
    for item in filter(None,changed):
        ...
        if not (name in {'public/index.html','public/stories/index.html','public/_headers','public/data/manifest.json'} or __import__('re').fullmatch(r'public/data/activity-\d{4}-\d{2}\.json',name)):
...
def main():
    ...
    receipt.update(publish=publish,revision=revision,manifest_sha256=hashlib.sha256((data/'manifest.json').read_bytes()).hexdigest())
