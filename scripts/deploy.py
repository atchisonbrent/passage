...
def assets():
    ...
    assert names == {p.relative_to(public).as_posix() for p in public.rglob("*") if p.is_file()}, (
...
def run():
    ...
    if args.mode == "check":
        ...
        return
    assert args.receipts and args.receipts.is_absolute(), (
        "Explicit absolute receipt directory required"
    )
    expected = set()
    if args.mode == "upload":
        expected = assets()
        assert not subprocess.check_output(["git", "status", "--porcelain"], cwd=ROOT).strip(), (
            "Deploy only a committed clean tree"
        )
    ...
    def api(path, method="GET", data=None, credential=None, content_type="application/json"):
        ...
        req = urllib.request.Request(
            ...
            method=method,
            headers={
                "Authorization": "Bearer " + (credential or auth),
    ...
    if args.mode == "status":
        ...
        save(
            "status.json",
            {
                "project": PROJECT,
                "deployment": {
                    k: deployment.get(k) for k in ["id", "latest_stage", "uses_functions"]
                },
                "domains": [{k: d.get(k) for k in ["name", "status"]} for d in domains],
            },
        )
        return
    public = ROOT / "public"
        ...
        batch.append(
            {
                ...
                "metadata": {"contentType": mime + "; charset=utf-8"},
                "base64": True,
            }
        )
        size += len(b)
    ...
    assert (
        api("/pages/assets/check-missing", "POST", {"hashes": list(manifest.values())}, jwt) == []
    ), "Assets were not confirmed present"
    ...
    for name, value in {
        ...
        "commit_dirty": "false",
        "commit_message": "Publish reviewed maritime atlas",
    }.items():
        parts.append(
    ...
    save(
        "deployment.json",
        {
            **{
                k: result.get(k)
                for k in ["id", "url", "environment", "latest_stage", "uses_functions"]
            },
            "commit": revision,
            "sha256": receipts,
        },
