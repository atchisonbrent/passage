...
The globe and comparison workspace both cover 2019 onward. `scripts/timeline.py` derives ignored measure/year packs from tracked per-place history during the build. `timeline.js` loads the full active historical measure into a dense Float64Array before enabling scrubbing; date changes do not fetch data. Two measures remain parsed, while low-priority idle fetches warm the others in the HTTP cache (skipped with Save-Data). Hash-versioned requests prevent stale history reuse. Existing recent partitions remain resident; selected-place history supplies complete chart/category rows. Errors remain visible and retryable in the existing map subtitle.
