...
class PublicationTests(unittest.TestCase):
    def test_cloudflare_appendages_are_narrow_and_inert(self):
        ...
        for actual,csp in [
            ...
            with self.subTest(actual=actual),self.assertRaises(ValueError):verify.verify_html(page,actual,csp,policy)
    ...
    def test_additions_cannot_reuse_an_authorized_hash(self):
        ...
        for extra in [b'<script>'+body+b'</script>', ('<script src="https://static.cloudflareinsights.com/beacon.min.js" integrity="'+token+'"></script>').encode()]:
            with self.assertRaisesRegex(ValueError,'authorized'):verify.verify_html(page,page.replace(b'</body>',extra+b'</body>'),policy,policy)
