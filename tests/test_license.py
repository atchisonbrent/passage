...
class LicenseTests(unittest.TestCase):
    def test_terms_are_required_and_structured(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            for name in ['LICENSE','LICENSES.md','public/data/LICENSE.txt','tests/fixtures/LICENSE.txt']:
                p=root/name;p.parent.mkdir(parents=True,exist_ok=True);p.write_text('license')
            with self.assertRaises(ValueError):check.validate(root)
