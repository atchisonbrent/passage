...
class SnapshotTests(unittest.TestCase):
    ...
    def test_counts_and_missing_semantics(self):
        ...
        seen=set();counts={'port':0,'chokepoint':0};latest={};ids=set()
        for part in manifest['activity']:
            b=(self.data/part['file']).read_bytes()
            self.assertEqual(hashlib.sha256(b).hexdigest(),part['sha256'])
            rows=json.loads(b);self.assertEqual(len(rows),part['rows'])
            for row in rows:
                self.assertEqual(len(row),9)
                key=tuple(row[:2]);self.assertNotIn(key,seen);seen.add(key);ids.add(row[0])
                kind='chokepoint' if row[0].startswith('chokepoint') else 'port'
                counts[kind]+=1;latest[kind]=max(latest.get(kind,''),row[1])
                self.assertTrue(all(v is None or isinstance(v,(int,float)) for v in row[2:]))
                if kind=='port': self.assertIsNone(row[8])
                else: self.assertEqual(row[6:8],[None,None])
    ...
    def test_network_and_model(self):
        ...
        self.assertEqual(hashlib.sha256((self.data/'network.json').read_bytes()).hexdigest(),self.manifest['network']['sha256'])
        for edge in network['edges']:
            self.assertIn(edge[0],network['nodes']);self.assertIn(edge[1],network['nodes'])
        for case in self.manifest['exposure']:
            b=(self.data/case['file']).read_bytes();self.assertEqual(hashlib.sha256(b).hexdigest(),case['sha256'])
            rows=json.loads(b);self.assertEqual(len(rows),case['rows'])
    ...
    def test_public_boundary_and_hashes(self):
        ...
        for receipt in self.manifest['assets']:
            self.assertEqual(hashlib.sha256((self.data/receipt['file']).read_bytes()).hexdigest(),receipt['sha256'])
