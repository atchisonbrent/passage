...
const places=[{id:'a',name:'Singapore'},{id:'b',name:'Other'}];
assert.equal(D.landing(places),'a');
assert.equal(D.landing(places.slice(1)),'b');
const pool=D.candidates([{id:'e',places:['a']}],[{id:'s',place:'b',qualifies:true},{id:'old',place:'a',qualifies:false}],places);
assert.equal(pool.length,2);
assert.deepEqual(pool.map(e=>e.key),['event:e','shift:s']);
