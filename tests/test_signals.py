...
class SignalsTests(unittest.TestCase):
    def rows(self):
        return [[str(dt.date(2026,1,1)+dt.timedelta(days=i)),100 if i<28 else 10] for i in range(35)]
    ...
    def test_gaps_zero_and_floor(self):
        ...
        with self.assertRaises(ValueError):signals.strongest(self.rows()+[self.rows()[0]])
        malformed=self.rows();malformed[0][0]='20260101'
        with self.assertRaises(ValueError):signals.strongest(malformed)
        self.assertEqual(signals.strongest(list(reversed(self.rows()))),signals.strongest(self.rows()))
    ...
    def test_pinned_source_windows(self):
        ...
        hits=[dict(place=place,context=next(c for c in contexts if c['place']==place),**signals.strongest(rows)) for place,rows in fixture.items()]
        expected={'chokepoint6':('2026-03-02','2026-03-08'),'port744':('2026-03-02','2026-03-08'),'port1253':('2026-07-08','2026-07-14')}
    ...
    def test_recent_window_does_not_reuse_old_winner(self):
        ...
        rows=self.rows()+[[str(dt.date(2026,1,1)+dt.timedelta(days=i)),100] for i in range(35,100)]
