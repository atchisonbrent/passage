...
const source = fs
  ...
  .match(/^function play\(\)\s*\{[\s\S]*?^\}/m)?.[0];
assert.ok(source, 'Actual play entrypoint must be found');
const shared = dates.indexOf(
...
for (const [index, expected] of [
  ...
  const context = {
    state: { index, mode: 'change', playing: false },
    manifest,
    dates,
    stop() {},
    refresh() {},
  ...
  assert.equal(
    context.state.index,
    expected,
    'Play at a latest snapshot must replay, not immediately end',
  );
  assert.equal(context.state.playing, true);
}
console.log(
  'actual play entrypoint: shared-latest replay, end replay, mid-range continuation passed',
);
